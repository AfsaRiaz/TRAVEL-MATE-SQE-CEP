...
if(isset($_POST['proceed'])){
   if ($_POST['payment'] == 'by_hand'){
        $full_name = $_SESSION['full_name'];
        $phone_number = $_SESSION['phone_number'];
        $pick_up_time = $_SESSION['pick_up_time'];
        $pick_up_date = $_SESSION['pick_up_date'];
        $pick_up_location = $_SESSION['pick_up_location'];
        $drop_off_location = $_SESSION['drop_off_location'];
        $vehicle_model = $_SESSION['vehicle_model'];
        $ride_distance = $_SESSION['distance'];
        $ride_cost = $_SESSION['cost'];
        $payment_method = 'by_hand';

        // echo $pick_up_date;
        $sql = "INSERT INTO bookings values('','$full_name','$phone_number', '$pick_up_time','$pick_up_date','$pick_up_location','$drop_off_location','$vehicle_model','$ride_distance','$ride_cost','$payment_method')";

        if ($conn->query($sql) === TRUE) {
          $last_insert_id = $conn->insert_id; // Get the auto-incremented ID
 ...
 <section id="center" class="center_reg">
  ...
  <div class="row center_o1">
   <div class="col-md-12">
	  ...
	  <h6 class="mb-0 mt-3 fw-normal col_oran"><a class="text-light" href="#">Booking</a> <span class="mx-2 col_light">/</span> Payment Method</h6>
   </div>
  </div>
 </div>
   </div>
...
<section id="login" class="p_3">
   ...
   <div class="row">
    <div class="col-md-12">
     <div class="login_1 p-4 m-auto">
       <!-- fields -->
    <h3 class="col_oran">Payment Method
    </h3>
    ...
    <div class="row">
     <!-- hand payment -->
     <div class="col-md-12">
      <h4 class="mt-1 mb-2"  style="color:#000000ba">Ride Rent: <?php echo 'Rs.'. $_SESSION['cost'];?></h4>
      <h6 class=" mb-4"  style="color:#000000ba">Total Distance: <?php echo $_SESSION['distance'].' km';?></h6>
     </div>
      <div class="col-md-12">
         ...
         <div class="d-flex flex-row pb-3">
           <div class="rounded border d-flex w-100 px-3 py-2 align-items-center">
             <div class="d-flex align-items-center pe-3">
              <input class="form-check-input" type="radio" name="payment" id="radioNoLabel22"required
                 value="by_hand" aria-label="..." />
              
             </div>
             <div class="d-flex flex-row align-items-center">
                 <h6 class="mb-0 pe-1">Hand Cash Payment</h6>
             </div>
           </div>
         </div>
      ...
      <div class="col-md-12">
        ...
        <div class="d-flex flex-row pb-3">
          <div class="rounded border d-flex w-100 px-3 py-2 align-items-center">
            <div class="d-flex align-items-center pe-3">
             <input class="form-check-input" type="radio" name="payment" id="radioNoLabel22" required
                value="by_card" aria-label="..." />
             
            </div>
            <div class="d-flex flex-row align-items-center">
                <h6 class="mb-0 pe-1">Card Payment</h6>
            </div>
          </div>
        </div>
 ...
 <!-- login button -->
     <!-- <h6 class="mt-4 mb-0 text-center"><a class="button" href="#">Proceed  <i class="fa fa-sign-in ms-1"></i></a></h6>
  -->
   </div>
    </div>
   </div>  
